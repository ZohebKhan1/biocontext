...
export const ConnectWizard = (props: ConnectWizardProps) => {
	...
	const proceedToModelSelection = async (providerId: string) => {
		...
		if (curated.length > 0) {
			...
			const currentReasoningEffort =
				...
				(currentModel === 'gpt-5.6-luna' ? ('medium' as const) : undefined);
			const currentIndex = options.findIndex(
				(option) =>
					option.kind === 'curated' &&
					option.id === currentModel &&
					(option.providerOptions?.reasoningEffort ?? undefined) === currentReasoningEffort
			);
	...
	const updateModel = async (
		...
		config.setReasoningEffort(providerOptions?.reasoningEffort);
		await config.refreshRuntimeStatus();
		...
		if (currentStep === 'model') {
			switch (key.name) {
				...
				case 'return': {
					...
					void updateModel(selectedProviderId, selected.id, selected.providerOptions);
					break;
				}
			}
		}
