...
describe('stripUserQuestionFromStart', () => {
	...
	test('strips question with @mentions', () => {
		const response = '@DESeq2 how does lfcShrink work? lfcShrink applies...';
		const question = '@DESeq2 how does lfcShrink work?';
		expect(stripUserQuestionFromStart(response, question)).toBe('lfcShrink applies...');
	});

	test('strips question when AI removes @mentions', () => {
		const response = 'how does lfcShrink work? lfcShrink works by...';
		const question = '@DESeq2 how does lfcShrink work?';
		expect(stripUserQuestionFromStart(response, question)).toBe('lfcShrink works by...');
	});
