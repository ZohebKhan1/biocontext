...
export interface ToolChunk {
	...
	/** Original tool input, retained so the TUI can show a compact target. */
	input?: unknown;
	/** Client timestamps used to keep very fast tool activity visible in the TUI. */
	startedAt?: number;
	completedAt?: number;
}
...
export type Chunk = TextChunk | ReasoningChunk | ToolChunk | FileChunk;
