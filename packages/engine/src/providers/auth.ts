...
export const getProviderAuthHint = (providerId: string) => {
	switch (providerId) {
		case 'github-copilot':
			return 'Run biocontext, use /connect, and complete GitHub Copilot device flow OAuth.';
		case 'openai':
			return 'Run "opencode auth --provider openai" and complete OAuth.';
		case 'openai-compat':
			return 'Set baseURL and name through /connect and optionally add an API key.';
		case 'anthropic':
		...
		case 'minimax':
			return 'Use /connect and enter your MiniMax API key. Get it at https://platform.minimax.io/user-center/basic-information.';
		default:
			return 'Run biocontext, use /connect, and configure credentials for this provider.';
	}
};
