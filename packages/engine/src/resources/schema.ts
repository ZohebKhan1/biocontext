...
/**
 * Bioconductor package name rules: start with a letter, then letters, digits,
 * and dots only. Matches R's own package naming constraints.
 */
const BIOCONDUCTOR_PACKAGE_REGEX = /^[a-zA-Z][a-zA-Z0-9.]*$/;

export const RPackageNameSchema = z
	.string()
	.min(1, 'R package cannot be empty')
	.max(LIMITS.RESOURCE_NAME_MAX, `Package name too long (max ${LIMITS.RESOURCE_NAME_MAX} chars)`)
	.regex(
		BIOCONDUCTOR_PACKAGE_REGEX,
		'R package must start with a letter and contain only letters, numbers, and dots'
	);

const BioconductorPackageSchema = RPackageNameSchema;

const BioconductorReleaseSchema = z
	.string()
	.regex(/^(release|devel|[0-9]+\.[0-9]+)$/, 'Release must be "release", "devel", or e.g. "3.20"')
	.optional();

/**
 * Legacy source configuration. Boolean values remain readable for compatibility
 * but are ignored because runtime source retrieval is mandatory. A URL selects
 * an explicit `custom_git` repository instead of the published repository.
 */
const BioconductorSourceSchema = z.union([z.boolean(), GitUrlSchema]).optional();

/**
 * Which published documents to download for a package.
 *
 * Omitting the field means every type, so existing configs keep working and the
 * default stays "everything Bioconductor publishes".
 */
export const BIOCONDUCTOR_DOCUMENT_TYPES = ['vignettes', 'vignetteScripts', 'manual', 'news'] as const;
export type BioconductorDocumentType = (typeof BIOCONDUCTOR_DOCUMENT_TYPES)[number];

const BioconductorDocumentsSchema = z
	.array(z.enum(BIOCONDUCTOR_DOCUMENT_TYPES))
	.min(1, 'Select at least one document type')
	.optional();

export const BioconductorResourceSchema = z.object({
	type: z.literal('bioconductor'),
	name: ResourceNameSchema,
	package: BioconductorPackageSchema,
	release: BioconductorReleaseSchema,
	documents: BioconductorDocumentsSchema,
	includeCurated: z.boolean().optional(),
	source: BioconductorSourceSchema,
	sourceBranch: BranchNameSchema.optional(),
	sourceCommit: z
		.string()
		.regex(/^[0-9a-f]{40}$/iu, 'sourceCommit must be a full 40-character Git commit')
		.optional(),
	specialNotes: SpecialNotesSchema
});

export const CranResourceSchema = z.object({
	type: z.literal('cran'),
	name: ResourceNameSchema,
	package: RPackageNameSchema,
	specialNotes: SpecialNotesSchema
});
...
export type BioconductorResource = z.infer<typeof BioconductorResourceSchema>;
export type CranResource = z.infer<typeof CranResourceSchema>;
...
export const isBioconductorResource = (value: ResourceDefinition): value is BioconductorResource =>
	value.type === 'bioconductor';

export const isCranResource = (value: ResourceDefinition): value is CranResource =>
	value.type === 'cran';

export const isBioconductorPackageName = (value: string): boolean => BIOCONDUCTOR_PACKAGE_REGEX.test(value.trim());

/** Resolve the requested document set, treating "unset" as every type. */
export const resolveBioconductorDocuments = (
	documents: readonly BioconductorDocumentType[] | undefined
): readonly BioconductorDocumentType[] => documents ?? BIOCONDUCTOR_DOCUMENT_TYPES;
